...
export function solutionCostEfficiencyComparison(efficiencyBarScaleFactor) {
  let processedSolutionData = processCostEfficiencyComparison(solutionData);
...
export function solutionGreenhouseGasReduction() {
  ...
  if (processedSolutionData.length > 0) {
    ...
    if (processedSolutionData.length > 0) {
      processedSolutionData[1].displayedValue += 'ons carbon dioxide equivalent/year'; //adjust index [] value to the position you want the string
   }
  return processedSolutionData;
}
