...
function processData(data) {
  return data.map((item) => {
    ...
    const formatNumber = (num) => {
      ...
      if (parseFloat(formattedNumber) === Math.round(parseFloat(formattedNumber))) {
        return Math.round(parseFloat(formattedNumber)).toLocaleString();
      } else {
        formattedNumber = parseFloat(formattedNumber).toFixed(5 - Math.floor(Math.log10(Math.abs(parseFloat(formattedNumber)))));
    ...
    const displayedValue = `${barlength >= 0 ? "$" : "-$"}${formatNumber(Math.abs(barlength))}`;
