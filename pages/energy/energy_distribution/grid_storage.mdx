...
# Electricity Storage
Energy storage might achieve [load balancing](http://urbancruiseship.org/solution/solution/load_balancing) through charging a storage device at times of low electricity prices and discharging at high prices, but doing so cost-effectively and at scale remains a challenge.
...
#### Source: Jenkins and Sepulveda [^13]. Seasonal storage with ammonia is expected to be less than the cost of hydrogen storage [^3][23]. See also our discussion of seasonal storage with [ammonia](http://urbancruiseship.org/solution/energy/ammonia).
